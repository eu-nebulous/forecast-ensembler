...
class BaseEnsembler:
    ...
    def fill_nans(data_frame: pd.DataFrame, columns):
        ...
        logger.debug(f"[fill_nans] Before filling, NaN counts per column:\n{data_frame[columns].isna().sum()}")
        data_frame[columns] = (
            data_frame[columns]
...
class AverageEnsembler(BaseEnsembler):
    ...
    def predict(self, msg_body):
        ...
        logger.info("[AverageEnsembler] predict() called")
        logger.debug(f"[AverageEnsembler] msg_body: {msg_body}")
        ...
        logger.debug(f"[AverageEnsembler] Extracted predictions: {predicted_values}")

        prediction = np.mean(np.array(predicted_values)) if predicted_values else 0.0
        logger.info(f"[AverageEnsembler] Final prediction: {prediction}")
        return prediction
...
class BestSubsetEnsembler(BaseEnsembler):
    ...
    def select_top_k(self, df: pd.DataFrame):
        ...
        """
        logger.info("[BestSubsetEnsembler] select_top_k() called")
    ...
    def train(self, historical_df):
        ...
        df = historical_df.df
        logger.debug(f"[BestSubsetEnsembler] DataFrame columns: {list(df.columns)}")

        # Find columns ending with '.prediction'
        self.prediction_columns = [col for col in df.columns if re.search(r"prediction$", col)]
        logger.info(f"[BestSubsetEnsembler] Found prediction columns: {self.prediction_columns}")
        ...
        logger.debug(f"[BestSubsetEnsembler] Generated {len(all_top_k_subsets)} subsets from top_k")
        ...
        for subset in all_top_k_subsets:
            if subset:  # skip empty subset
                ...
                logger.debug(f"[BestSubsetEnsembler] Subset={subset}, MAE={mae}")

                if best_mae is None or mae < best_mae:
                    best_mae = mae
                    best_subset = subset

        logger.info(f"[BestSubsetEnsembler] Best subset: {best_subset}, MAE={best_mae}")
        ...
        logger.info(f"[BestSubsetEnsembler] Selected forecasters: {self.selected_forecasters}")
        self.last_update_time = int(time.time())
        self.available = True

    def predict(self, msg_body):
        """Combine selected forecasters' predictions by average."""
        logger.info("[BestSubsetEnsembler] predict() called")
        logger.debug(f"[BestSubsetEnsembler] msg_body: {msg_body}")
        ...
        logger.debug(f"[BestSubsetEnsembler] Constructed predictions DataFrame:\n{predicted_values}")

        # Fill NaNs in all forecasters
        predicted_values = self.fill_nans(predicted_values, self.forecasters)
        logger.debug(f"[BestSubsetEnsembler] DataFrame after fill_nans:\n{predicted_values}")

        # If no selected forecasters, return 0.0 or a default
        if not self.selected_forecasters:
            logger.info("[BestSubsetEnsembler] No selected forecasters, returning 0.0")
            return 0.0
        ...
        logger.info(f"[BestSubsetEnsembler] Final prediction: {final_pred}")
        return final_pred
...
class LinnearProgrammingEnsembler(BaseEnsembler):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.weights = None
        self.prediction_columns = []
        logger.info("[LinnearProgrammingEnsembler] Initialized")

    def train(self, historical_df):
        """Derive weights from historical predictions by solving a linear least-squares problem."""
        logger.info("[LinnearProgrammingEnsembler] train() called")

        if not hasattr(historical_df, "df") or not hasattr(historical_df.df, "columns"):
            raise TypeError("Expected 'historical_df' to have a '.df' attribute that is a DataFrame.")

        df = historical_df.df
        logger.debug(f"[LinnearProgrammingEnsembler] DataFrame columns: {list(df.columns)}")

        self.prediction_columns = [
            col for col in df.columns if re.search(r"prediction$", col)
        ]
        logger.info(f"[LinnearProgrammingEnsembler] Found prediction columns: {self.prediction_columns}")

        # Fill missing values, keep last N
        df = self.fill_nans(df, self.prediction_columns)
        df = df.tail(self.last_n_rows)
        logger.debug(f"[LinnearProgrammingEnsembler] Using last {self.last_n_rows} rows => shape: {df.shape}")

        # Prepare design matrix X (forecasters) and target vector y
        X = df[self.prediction_columns].to_numpy()
        y = df["y"].to_numpy()

        logger.debug(f"[LinnearProgrammingEnsembler] X shape={X.shape}, y shape={y.shape}")
        ...
        logger.info(f"[LinnearProgrammingEnsembler] Learned weights: {self.weights}")
        self.last_update_time = int(time.time())
        self.available = True

    def predict(self, msg_body):
        """Use the learned linear weights to combine forecasters' predictions."""
        logger.info("[LinnearProgrammingEnsembler] predict() called")
        logger.debug(f"[LinnearProgrammingEnsembler] msg_body: {msg_body}")
        ...
        logger.debug(f"[LinnearProgrammingEnsembler] Predictions DataFrame:\n{predicted_values}")

        predicted_values = self.fill_nans(predicted_values, self.forecasters)
        logger.debug(f"[LinnearProgrammingEnsembler] After fill_nans:\n{predicted_values}")

        if self.weights is None:
            logger.warning("[LinnearProgrammingEnsembler] Weights are None, defaulting to mean.")
        ...
        logger.info(f"[LinnearProgrammingEnsembler] Final prediction: {final_pred}")
        return final_pred